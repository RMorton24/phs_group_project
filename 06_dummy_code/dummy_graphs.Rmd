...
bed_trends <- bed_data %>% 
  ...
  ggplot() +
  geom_col(aes(x = year_quarter, y = average_beddays), fill = "violet") +
    theme(axis.text.x = element_text(angle = 90))
...
bed_trends_occu <- bed_data %>% 
  ...
  ggplot() +
  geom_col(aes(x = year_quarter, y = average_occupancy)) +
    theme(axis.text.x = element_text(angle = 90))
...
episodes_per_specialty <- specialty_data %>% 
  ...
  summarise(total_episodes = sum(episodes)) %>%
  ggplot() +
  geom_col(aes(x = specialtsy_name, y = total_episodes)) +
  theme(axis.text.x = element_text(angle = 90))
...
Hospitalisation_trends <- admission_data2 %>% 
  select(month_name, hb_name, year, admission_type, quarter) %>% 
  filter(admission_type == "Emergency", year >= 2020) %>% 
  group_by(quarter) %>% 
  summarise(total_deaths = sum(number_admissions)) %>% 
  ggplot() +
  geom_line(aes(x = quarter, y = total_deaths))

Hospitalisation_differ <- admission_data2 %>% 
  select(month_name, hb_name, year, admission_type) %>% 
  filter(year >= 2020) %>% 
  group_by()
```
